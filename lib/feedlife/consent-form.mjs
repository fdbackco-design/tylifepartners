  ...
  dialog.innerHTML = `
    ...
    <form id="fl-form" novalidate>
      ...
      <section class="fl-consent-section" aria-label="상담 동의">
        <div class="fl-consent-item"><label class="fl-check" for="fl-required"><input id="fl-required" name="requiredPrivacy" type="checkbox" required><span>${CONSENT_COPY.requiredTitle}</span></label><details><summary>수집 항목·목적·보유기간 확인</summary><div class="fl-terms">${paragraphs(CONSENT_COPY.required)}</div></details></div>
        <div class="fl-consent-item"><label class="fl-check" for="fl-optional"><input id="fl-optional" name="optionalConsultation" type="checkbox"><span>${CONSENT_COPY.optionalTitle}</span></label><details><summary>맞춤 상담 동의 내용 확인</summary><div class="fl-terms">${paragraphs(CONSENT_COPY.optional)}</div></details>
          <fieldset id="fl-extra" class="fl-fields fl-extra" aria-label="맞춤 상담 추가정보" disabled>
